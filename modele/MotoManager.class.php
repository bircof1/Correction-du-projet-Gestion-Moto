...
class MotoManager
{
    ...
    public function get($id)
   {
    ...
    {
        $resq=$this->_db->prepare("DELETE FROM moto WHERE id=?");
        $resq->execute(array($id));
    }

    public function update(Moto $moto)
    { 
                $sql=$this->_db->prepare("UPDATE moto SET cathegorie=:cathegorie,marque=:marque,model=:model,cylindre=:cylindre,
                numero_seri=:numero_seri,prix=:prix WHERE id=:id");
                $sql->execute(array("cathegorie"=>$moto->getCathegorie(),
